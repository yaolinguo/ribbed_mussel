...
ft <- width(ft, j = 1:9,
            width = c(1.084, 1.279, 1.085, 0.592, 0.592, 0.590, 0.685, 0.496, 0.369))
...
sup <- fp_text(font.family = "Times New Roman", font.size = 11, italic = TRUE,
...
make_panel <- function(tr, e, show_x, show_strip) {
  ...
  d_sig <- d %>% filter(Dens %in% st$Dens[st$sig])   # only significant lines
