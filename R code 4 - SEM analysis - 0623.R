...
# -------------------------
# standardized path coefficients (data are pre-z-scored, so Estimate = standardized)
co <- as.data.frame(coefs(pw_mod_updated))   # Response, Predictor, Estimate, Std.Error, DF, Crit.Value, P.Value, ...
...
sig_rows <- which(co$P.Value < 0.05)   # whole row bold when significant
