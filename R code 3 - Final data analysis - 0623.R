...
# -------------------------
# all lmms Table S5
setwd("/Users/yaolin/Desktop/Postdoc in LSU/Project - Ribbed Mussels Ecology/Submission - Journal of Applied Ecology - 0623 - Final version/GitHub")
...
n_plant <- 14                          # first 14 = plant traits; last 2 = soil traits

fmt_chi <- function(x) sprintf("%.3f", x)
fmt_p   <- function(p) ifelse(p < 0.001, "< 0.001", sprintf("%.3f", p))
terms   <- c("Level", "Treat", "Level:Treat")          # E, D, E x D

stat <- vector("list", length(resp_vars))
sig  <- matrix(FALSE, length(resp_vars), 3,
               dimnames = list(NULL, c("E","D","I")))

for (i in seq_along(resp_vars)) {
  f <- as.formula(paste0(resp_vars[i], " ~ Level*Treat + (1 | Organ)"))
  m <- lmer(f, data = data, na.action = na.omit)
  a <- as.data.frame(car::Anova(m))                    # Type II Wald chi-square (car default)
...
sec_rows <- c(1, n_plant + 2)                                   # Plant traits, Soil traits
var_brow <- c(1:n_plant + 1, (n_plant+1):length(resp_vars) + 2) # body row of each variable
...
ft <- align(ft, i = 1, align = "center", part = "header")   # group labels centered
...
elevs    <- c(5, 0, -5, -10, -15)
elev_lab <- c("+5", "0", "\u20135", "\u201310", "\u201315")   # en dash for negatives
rows <- list()
for (v in names(vars)) {
  for (j in seq_along(elevs)) {
    sub <- d[d$Elevation == elevs[j], ]
    fit <- lm(reformulate("Mussel_density", v), data = sub)   # NA rows dropped by default
...
sig <- which(res$p < 0.05)   # rows to bold
...
sup <- fp_text(font.family = "Times New Roman", font.size = 10, italic = TRUE,
...
# Figure 3 & S3 & S4
# -------------------------
# ---- definitions required by this section (added) ----
...
x_breaks_raw  <- c(0, 2, 4, 8)                   # Mussel_density 的原始取值
x_labels_show <- c("0", "254", "509", "1018")    # 对应的 per-m² 标签
